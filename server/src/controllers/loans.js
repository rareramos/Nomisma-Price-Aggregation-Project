...
export const getLoanTokens = async () => {
...
export const getLoanVolumeBinDataPoints = async ({
  ...
  if (!lastTimestampObjArr.length) {
    ...
    const hasPlatformFilter = platform !== 'all';
    const matchParams = { updateBlock: lastTimestampObjArr[0].currentBlock };
    if (hasPlatformFilter) {
      matchParams.platform = platformFilters[platform];
    }
    ...
    const boundaries = new Array(boundariesAmt - 1)
      .fill()
      .map(
        (
          _,
          idx,
        ) => startTime + (idx * durationPerBucket)
      );
