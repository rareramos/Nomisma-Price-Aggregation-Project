...
const processDharmaHash = async ({
  ...
  return dharmaDataArr.reduce(
    async (
      ...
      if (!token) {
        log.error({
          message: `Failed to find token ${debtOrderFilled._principalToken}`,
        });
        ...
        let ccr;
        let collateralInBase;
        if (collateralAdded) {
          collateralInBase = collateralAdded.amount || '0';
          ...
          if (collateralToken) {
            ...
            log.error({
              message: `Failed to find collateral token ${collateralAdded.token}`,
            });
          }
        } else {
          collateralSymbol = null;
        ...
        const newItem = {
          ...
          collateralSymbol,
          ccr,
          interestInBase: new BN(currentState.expectedRepaymentValue)
          ...
          principalInBase: debtOrderFilled._principal,
          repaidInBase: currentState.repaidToDateValue,
          allCollaterals: [{
            token: collateralSymbol,
            amount: collateralInBase,
          }],
...
const saveDharmaItems = async (items) => {
  ...
  log.debug({
    message: `Saving ${itemsWithBlock.length} Dharma loan records to db.`,
  });
